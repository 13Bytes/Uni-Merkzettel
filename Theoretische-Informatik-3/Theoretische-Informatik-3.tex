...
\begin{multicols}{3}
    ...
    Ähnlich zu CYK. $T_{i,j} = \min(T_{i,j} + T_{m+1, j} +  n_{i-1}\cdot n_m \cdot n_j)$\\
    ...
    $C_n$ gibt Anzahl saturierter Binärbäume mit $n$ inneren Knoten an
    \hrule
    \smallskip
    ...
    Alle können kommutativ sein ($x\circ y = y \circ x$) \textcolor{gray}{(gilt nicht für Minus)}
